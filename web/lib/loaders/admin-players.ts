...
export interface AdminDivisionDetailData {
  ...
  // Net life differential per player across the division's confirmed regular-season
  // games: +winnerLives when they won a game, −winnerLives when an opponent beat
  // them. Shown next to the resolve-tie form as the tiebreaker reference.
...
export async function loadAdminDivisionDetail(
  ...
  // Net life differential per player (tiebreaker reference for the resolve-tie
  // form). Only confirmed regular-season (BO2) games with a captured winnerLives.
