...
const CHANNEL_KEYS = [
  ...
  { key: "bot_commands_channel_id", label: "Bot-commands channel ID(s)", help: "Where public bot commands (/random, /pool) are allowed to run. Accepts a COMMA-SEPARATED LIST of channel IDs to allow several channels. The admin channel is always allowed too. Ephemeral commands (/standings, /profile, /schedule) and dedicated-channel ones (/challenge) still work anywhere. Auto-created as #league-bot-commands on startup if unset." },
  ...
  { key: "help_channel_id", label: "Help channel ID", help: "Read-only #league-help channel where the bot pins the player command list. Auto-created by /league setup; re-running setup refreshes the pinned message." },
...
const BMP_KEYS = [
  ...
  { key: "dm_panels_enabled", label: "Player DM panels enabled", help: "Set to 'true' to start DM'ing each player a personal panel (season deadline, their standing, and the quick-action buttons), refreshed hourly by editing that same message in place - no repeat notifications. Leave unset/'false' to keep it off. Turn this on at season kickoff." },
