...
export async function listSeasonNews(seasonName: string): Promise<NewsPostView[]> {
  ...
  if (!sid) return [];
  const posts = await prisma.newsPost.findMany({ where: { seasonId: sid }, orderBy: [{ postedAt: "desc" }, { createdAt: "desc" }] });
  return posts.map((p) => ({ id: p.id, week: p.week, title: p.title, body: p.body, createdBy: p.createdBy, postedAt: p.postedAt }));
}
...
export async function createNews(seasonName: string, data: { week: number | null; title: string; body: string; by?: string; postedAt?: Date | null }) {
  const sid = await seasonIdOf(seasonName);
  ...
  return prisma.newsPost.create({ data: { seasonId: sid, week: data.week, title: data.title.trim(), body: data.body, createdBy: data.by ?? null, ...(data.postedAt ? { postedAt: data.postedAt } : {}) } });
}

export async function updateNews(id: string, data: { week: number | null; title: string; body: string; postedAt?: Date | null }) {
  if (!data.title.trim()) throw new Error("A title is required.");
  if (!data.body.trim()) throw new Error("The post is empty.");
  return prisma.newsPost.update({ where: { id }, data: { week: data.week, title: data.title.trim(), body: data.body, ...(data.postedAt ? { postedAt: data.postedAt } : {}) } });
}
