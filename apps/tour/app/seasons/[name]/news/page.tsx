      ...
      {posts.length === 0 ? (
        ...
        posts.map((p) => (
          <article className="card" key={p.id}>
            ...
            <p className="sub" style={{ marginTop: 2 }}>{fmtDate(p.postedAt)}{p.createdBy ? ` · ${p.createdBy}` : ""}</p>
            <div className="post-body" dangerouslySetInnerHTML={{ __html: renderPostMarkdown(p.body) }} />
          </article>
        ))
      )}
    </main>
